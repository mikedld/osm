...
def fetch_data():
    return fetch_json_data(DATA_URL, headers={"x-nb-oc-channel": "1.2"})["data"]
...
if __name__ == "__main__":
    ...
    old_data = [
        DiffDict(e)
        for e in overpass_query('nwr[amenity=bank][name~"Novo Banco"][brand!="Novo Banco dos Açores"](area.country);')
    ]
    ...
    for nd in new_data:
        ...
        elif nd["tipoEstrutura"] in ("BL", "EXT"):
            d["name"] = d["name"] or "Novo Banco"
        elif nd["tipoEstrutura"] == "CE":
            d["name"] = d["name"] or "Novo Banco Empresas"
        else:
            d["name"] = f"<ERR:{nd['tipoEstrutura']}>"
